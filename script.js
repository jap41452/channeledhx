/* ==== Dynamic iframe height messenger (safe to prepend) ==== */
(function () {
  // Post current document height to the parent page.
  function postHeight() {
    var h = Math.max(
      document.documentElement.scrollHeight,
      document.body ? document.body.scrollHeight : 0
    );
    // Use "*" here; the parent will verify origin before applying.
    try {
      window.parent.postMessage({ type: "CHAN_HEIGHT", height: h }, "*");
    } catch (e) {
      // no-op
    }
  }

  // Debounce to avoid flooding messages during layout thrash
  ...
  // Watch for any DOM/size changes
  if (typeof ResizeObserver !== "undefined") {
    const ro = new ResizeObserver(requestPost);
    ro.observe(document.documentElement);
    if (document.body) ro.observe(document.body);
  } else {
    // Fallback: watch DOM mutations and also tick occasionally
    if (typeof MutationObserver !== "undefined") {
      const mo = new MutationObserver(requestPost);
      mo.observe(document.documentElement, { childList: true, subtree: true, attributes: true });
      if (document.body) mo.observe(document.body, { childList: true, subtree: true, attributes: true });
    }
    setInterval(requestPost, 800);
  }

  // Also expose a helper you can call after big UI updates:
  window.__notifyParentOfHeight = postHeight;
})();
